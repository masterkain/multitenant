  ...
  module SchemaUtils
    class << self
      ...
      def create_schema(schema_name)
        fail "#{schema_name} already exists" if schema_exists?(schema_name)
      ...
      def drop_schema(schema_name)
        fail "#{schema_name} does not exists" unless schema_exists?(schema_name)
      ...
      def load_schema_into_schema(schema_name)
        ...
        with_schema(schema_name) do
          ...
          if File.exist?(file)
            ...
            fail "#{file} desn't exist yet. It's possible that you just ran a migration!"
          end
        end
      end
